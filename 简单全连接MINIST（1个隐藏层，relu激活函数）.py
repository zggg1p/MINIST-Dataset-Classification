...
model = Batch_Net(28*28,300,10)  # 实例化一个网络对象
...
if __name__ == "__main__":
    ...
    print("Start Training, 简单全连接MINIST!")  # 定义遍历数据集的次数
    with open("简单全连接acc.txt", "w") as f:
        with open("简单全连接log.txt", "w")as f2:
            for epoch in range(pre_epoch, EPOCH):
                ...
                with torch.no_grad():
                    ...
                    if acc > best_acc:
                        f3 = open("简单全连接best_acc.txt", "w")
                        f3.write("EPOCH=%d,best_acc= %.3f%%" % (epoch + 1, acc))
                        f3.close()
                        best_acc = acc
            print('Saving model......')
            torch.save(model, '简单全连接MINIST_%03d.pth' % (epoch + 1))
